...
class FillJukeboxUnitTests(TestCase):
    ...
    def test_times_are_rounded(self):
        ...
        #
        # self.assertEquals([3, 1, 1, 3, 3], [r["id"] for r in res])
        # self.assertEquals(self.start_date + datetime.timedelta(minutes=1), res[0]["starttime"])
    ...
    def test_cases(self):
        ...
        #     video_tok = [re.match(r"(?P<id>\d):(?P<min>\d)m", x) for x in video_str.split(" ")]
        #     videos = [
        #         self._video(video_id=int(m.group("id")), min=int(m.group("min"))) for m in video_tok
        #     ]
        #
        #     sched_tok = [
        #         re.match(r"((?P<existing>x)|((?P<dur>\d)m))", x) for x in sched_str.split(" ")
        #     ]
        #     sched_cur = 0
        ...
        #         Scheduleitem(
        #             video_id=r["id"], starttime=r["starttime"], duration=r["video"].duration
        #         )
        #         for r in res
        #     ]
        ...
        #         self.assertEquals(expect_str, newly_scheduled_str)
