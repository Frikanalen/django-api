...
def make_slot(source: WeeklySlotSource | None, **fields) -> WeeklySlot:
...
def test_fills_every_occurrence_up_to_the_horizon(video: Video, source: WeeklySlotSource) -> None:
    make_slot(source)
...
def test_todays_occurrence_counts_only_while_still_ahead(
    video: Video, source: WeeklySlotSource
) -> None:
    ...
    passed = make_slot(source, day=3, start_time=time(11, 0))
    ahead = make_slot(source, day=3, start_time=time(13, 0))
...
def test_skips_slots_without_a_source(video: Video) -> None:
    make_slot(None)
...
def test_skips_slots_whose_source_has_no_eligible_video(
    editor: User, organization: Organization, source: WeeklySlotSource
) -> None:
    ...
    make_slot(source, duration=timedelta(hours=1))
...
def test_displaces_jukebox_fillers_in_the_open_week(video: Video, source: WeeklySlotSource) -> None:
...
def test_placements_carry_their_slot(video: Video, source: WeeklySlotSource) -> None:
    slot = make_slot(source)
...
def test_a_rerun_re_picks_the_open_week_when_the_source_answer_changed(
    editor: User, organization: Organization, video: Video, source: WeeklySlotSource
) -> None:
...
def test_a_random_slot_is_not_re_rolled_night_after_night(
    ...
    random_source = WeeklySlotSource.objects.create(
        name="Random source",
        type=SlotSourceType.ORGANIZATION,
        strategy="random",
        organization=organization,
    )
    make_slot(random_source)
    fill_next_weeks_agenda(now=NOW)
...
def test_a_drafted_video_that_became_ineligible_is_replaced(
    ...
    """A pick the source can no longer stand by -- here the video lost
...
def test_management_command_runs_the_filler(
    monkeypatch: pytest.MonkeyPatch, video: Video, source: WeeklySlotSource
) -> None:
